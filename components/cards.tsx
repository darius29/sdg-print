...
export const ServiceCard = ({ service }: { service: Service }) => (
  <article className="card-premium motion-base p-6">
    ...
    <h3 className="mt-4 font-heading text-xl font-semibold line-clamp-2">{service.title}</h3>
    <p className="mt-3 text-sm text-muted line-clamp-3">{service.shortDescription}</p>
    <Link href="/servicii" className="focus-ring mt-5 inline-block text-sm font-medium text-primary hover:text-accent">
      Detalii →
    </Link>
  </article>
);
  ...
  >
    <div className="relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-x-4 bottom-4 translate-y-2 opacity-0 transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
        <span className="inline-flex rounded-full border border-primary/40 bg-bg/70 px-3 py-1 text-xs font-medium text-primary">
          Vezi proiectul
        </span>
      </div>
    </div>
    <div className="p-5">
      ...
      <h3 className="mt-2 font-heading text-xl line-clamp-2">{item.title}</h3>
...
export const TestimonialCard = ({ item }: { item: Testimonial }) => (
  <article className="card-premium p-6">
    <p className="text-amber-400" aria-label={`${item.rating} stele din 5`}>
      {'★'.repeat(item.rating)}
    </p>
    <p className="mt-4 text-muted line-clamp-5">"{item.text}"</p>
